...
public class LogAspect {
    ...
    protected void handleLog(final JoinPoint joinPoint, Log controllerLog, final Exception e, Object jsonResult) {
        try {
            ...
            operateLog.setOperUrl(StringUtil.substring(ServletUtil.getRequest().getRequestURI(), 0, 255));
            if (loginUser != null) {
                operateLog.setOperName(loginUser.getUsername());
            }
            if (e != null) {
                operateLog.setStatus(BusinessStatus.FAIL.ordinal());
                operateLog.setErrorMsg(StringUtil.substring(e.getMessage(), 0, 2000));
            }
            // 设置方法名称
    ...
    public void getControllerMethodDescription(JoinPoint joinPoint, Log log, SysOperateLog operateLog, Object jsonResult) throws Exception {
        ...
        if (log.isSaveResponseData() && StringUtil.isNotNull(jsonResult)) {
            operateLog.setJsonResult(StringUtil.substring(JSON.toJSONString(jsonResult), 0, 2000));
        }
    }
    ...
    private void setRequestValue(JoinPoint joinPoint, SysOperateLog operateLog, String[] excludeParamNames) throws Exception {
        ...
        if (StringUtil.isEmpty(paramsMap)
            ...
            operateLog.setOperParam(StringUtil.substring(JSON.toJSONString(paramsMap, excludePropertyPreFilter(excludeParamNames)), 0, 2000));
        }
    }
